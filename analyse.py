...
def plot_scatter(m, title, out_path):
    ...
    ax.axvline(TABLE_WIDTH_CM / 2, color="#333333", linewidth=1, linestyle="--", alpha=0.4)
    ...
    # Lines from each landing to target
    for x, y in zip(m["lx"], m["ly"]):
        ax.plot([m["tx"], x], [m["ty"], y],
                color="gray", linewidth=0.6, alpha=0.5, zorder=3)

    # Landing positions
    ax.scatter(m["lx"], m["ly"],
               marker="x", color="#1f77b4", s=60, linewidths=1.5,
